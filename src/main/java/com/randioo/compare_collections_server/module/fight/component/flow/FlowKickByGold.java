...
public class FlowKickByGold implements Flow {
    ...
    public void execute(Game game, String[] params) {
        ...
        needGold = YiyaAutoBetConfigCache.getYiYaAutoBetMap().get(game.matchParameter).autoBet;
    ...
    private void kick(Game game, Map<String, KickReason> kickMap, Map<Integer, KickReason> kickAudiencesMap) {
        ...
        for (Map.Entry<Integer, KickReason> entry : kickAudiencesMap.entrySet()) {
            ...
            // .setSCMatchExitGame(SCMatchExitGame.newBuilder().setSeat(audienceManager.getSeat(roleId,
            // game)))
            // .build(), roleId);
            ...
            SessionUtils.sc(
                    roleId,
                    SC.newBuilder()
                            .setSCMatchKickGame(SCMatchKickGame.newBuilder().setReason(entry.getValue().getNumber()))
                            .build());
            // 移除
        ...
        for (String kickGameRoleId : kickMap.keySet()) {
            ...
            SessionUtils.sc(
                    ...
                    SC.newBuilder()
                            .setSCMatchKickGame(
                                    SCMatchKickGame.newBuilder().setReason(kickMap.get(kickGameRoleId).getNumber()))
                            .build());
        }
